...
to https://vccircle.com, log in, and then extract the session token,
cookies, and any other relevant authentication data from the browser's internal storage.
---
## **Task Instructions**

1.  **Initial Navigation**: Go to `https://vccircle.com`.
...
4.  **Login Process**:
    ...
    * Enter email: "info@riverwalkholdings.com" into the email field and continue.
    * **Assess the next screen for a password input field.**
        i. **If a password field is present:**
          1. Enter password: "VC@Riverwalk123" into the password field.
        ...
        ii. **If no password field is immediately present (implying an OTP-first flow):**
